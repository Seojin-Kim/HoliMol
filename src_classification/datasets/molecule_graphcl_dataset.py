...
class MoleculeDataset_graphcl(MoleculeDataset):
    ...
    def get(self, idx):
        ...
        if self.aug_mode == 'no_aug':
            ...
            data1 = data
            data2 = self.edge_del(data)
        elif self.aug_mode == 'molclr_before':
            augmentations = [self.subgraph, self.edge_del, self.attr_mask]
            n_aug = np.random.choice(3,1)[0]
            n_aug1 = np.random.choice(3,1)[0]
            n_aug2 = np.random.choice(3,1)[0]
            data = augmentations[n_aug](data.clone())
            data1 = augmentations[n_aug1](data1.clone())
            data2 = augmentations[n_aug2](data2.clone())
        elif self.aug_mode == 'fragcl':
            augmentations = [self.attr_mask, self.node_drop, lambda x:x]
            n_aug = np.random.choice(3,1)[0]
            n_aug1 = np.random.choice(3,1)[0]
            n_aug2 = np.random.choice(3,1)[0]
            data = augmentations[n_aug](data.clone())
