...
class GINConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        #print(x.shape, edge_index[0].shape, edge_embeddings.shape)
        #raise dd
...
class GNN_graphpred(nn.Module):
    ...
    def __init__(self, args, num_tasks, molecule_model=None):
        ...
        if self.JK == "concat":
            ...
            #pred_head = [nn.Linear(self.mult * self.emb_dim, self.emb_dim//2), nn.ReLU(inplace=True)]
            #pred_head.append(nn.Linear(self.emb_dim//2, self.num_tasks))
...
class GNN_graphpred_mae(nn.Module):
    """
    Extension of GIN to incorporate edge information by concatenation.
    ...
    def __init__(self, args, num_tasks, molecule_model=None, mae=None):
        super(GNN_graphpred_mae, self).__init__()
    ...
    def from_pretrained(self, model_file):
        ...
        self.mae.load_state_dict(torch.load(model_file)['mae'])
        return
    ...
    def do_mae(self, reps):
        reps, _, _ = self.mae.forward_encoder(reps, 0.0)
        reps = reps.mean(dim=1)
        
        output = self.graph_pred_linear(reps)
